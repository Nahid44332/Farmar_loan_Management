...
{
   public function riceAdmin($id)
   {
      $riceService = Service::findOrFail($id);
      $services = Service::all();
      return view('backend.rice.index', compact('riceService', 'services'));
   }

   public function riceUpdate(Request $request, $id)
   {
      $request->validate([
         'name'  => 'required',
      ...
      try {
         ...
         $service->save();
         return redirect()->back()->with('success', 'মামা, রাইস ডাটা একদম আপডেট হয়ে গেছে! 🔥');
      } catch (\Exception $e) {
   ...
   public function benefitCard($id)
   {
      ...
      return view('backend.rice.benefit', compact('riceService', 'benefits', 'services'));
   }
   ...
   public function riceFaq($id)
   {
      $faqs = Faq::where('service_id', $id)->get();
      $services = Service::all();
      $currentService = Service::findOrFail($id);
      return view('backend.rice.faq', compact('faqs', 'services', 'currentService'));
   }
