...
{
   public function maizeAdmin()
   {
      // মেইজ সাধারণত প্রথম সার্ভিস (ID: 1) হয়ে থাকে, তাই find(1) অথবা ডাইনামিক আইডি পাস করতে পারেন
      $about = Service::find(1) ?? Service::first();
      $services = Service::all();
      return view('backend.maize.index', compact('about', 'services'));
   }

   public function maizeUpdate(Request $request, $id)
   {
      $request->validate([
         'name'  => 'required',
      ...
      try {
         ...
         $service->save();
         return redirect()->back()->with('success', 'মামা, মেইজ ডাটা একদম আপডেট হয়ে গেছে! 🔥');
      } catch (\Exception $e) {
   ...
   public function maizeCard($id)
   {
      $currentService = Service::findOrFail($id);
      $services = Service::all();
      $benefits = Benefit::where('service_id', $id)->get();
      return view('backend.maize.banifit', compact('benefits', 'services', 'currentService'));
   }
   ...
   public function benefitDelete($id)
   {
      try {
         $benefit = Benefit::findOrFail($id);
         $benefit->delete();
         return back()->with('success', 'মামা, আইটেমটি সফলভাবে ডিলিট করা হয়েছে! 🗑️');
      } catch (\Exception $e) {
         return back()->with('error', 'মামা, ডিলিট করতে গিয়ে ঝামেলা হয়েছে।');
      }
   }

   public function maizeFaq($id)
   {
      $faqs = Faq::where('service_id', $id)->get();
      $services = Service::all();
      $currentService = Service::findOrFail($id);
      return view('backend.maize.faq', compact('faqs', 'services', 'currentService'));
   }
   ...
   public function update(Request $request, $id)
   {
      ...
      try {
         ...
         return back()->with('success', 'মামা, FAQ টা সাকসেসফুলি আপডেট হয়েছে! 🚀');
      } catch (\Exception $e) {
