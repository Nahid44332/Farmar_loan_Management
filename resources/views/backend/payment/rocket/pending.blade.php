...
<div class="p-6">
    <div class="flex justify-between items-center mb-6">
        <h2 class="text-2xl font-semibold text-gray-800">Rocket Pending Payments</h2>
                ...
                <tbody class="text-gray-700 text-sm divide-y divide-gray-100">
                    @forelse($requests as $row)
                        <tr class="hover:bg-gray-50/50 transition-colors">
                            ...
                            <td class="p-4 text-center"><span class="bg-amber-100 text-amber-700 text-xs px-2.5 py-1 rounded-full font-medium">Pending</span></td>
                            <td class="p-4 text-center">
                                <form action="{{ route('admin.rocket.approve', $row->id) }}" method="POST" onsubmit="return confirm('Approve this Rocket payment?');">
                                    @csrf
                    ...
                    @empty
                        <tr><td colspan="6" class="p-8 text-center text-gray-400 italic">No pending Rocket payments found.</td></tr>
                    @endforelse
                </tbody>
            </table>
        </div>
